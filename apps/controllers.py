...
@app.route('/init_list/<tag_id>', methods=['GET'])
def init_list(tag_id):

    tag = Tag.query.get(tag_id)

    list_string = ""
    if tag.schedule:  
        list_string = tag.schedule

    return list_string


@app.route('/save_list/<tag_id>')
def save_list(tag_id):
    list_str = request.args['list_string']

    # 해당 프로젝트의 해달 컬럼에 저장
    tag = Tag.query.get(tag_id)
    tag.schedule = list_str
    db.session.commit()

    return "success"

@app.route('/send/<tag_id>', methods=['GET'])
def sendmsg(tag_id):
    ...
    # 이 채널을 유동적으로.
    p[tag_id].trigger('chat_msg', {'name': chat_name, 'msg': chat_msg})

    return ""

@app.route('/add_list/<tag_id>', methods=['GET'])
def add_list(tag_id):
    ...
    # 이 채널을 유동적으로.
    p[tag_id].trigger('add_list', {'list_item': list_item})

    return ""

@app.route('/update_list/<tag_id>', methods=['GET'])
def update_list(tag_id):
